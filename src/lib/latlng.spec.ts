...
import { dd2dms, dms2dd, format, parse } from './latlng';
...
test('format', t => {
  ...
  t.is(format({degrees: 10, minutes: 7, seconds: 24.24, direction: 'N'}), '10° 7\' 24.24" N');
});

test('parse', t => {
  t.deepEqual(parse('10° 7\' 24.24"'), {degrees: 10, minutes: 7, seconds: 24.24});
  t.deepEqual(parse('10° 0\' 0"'), {degrees: 10, minutes: 0, seconds: 0});
  t.deepEqual(parse('10° 7\' 24.24" N'), {degrees: 10, minutes: 7, seconds: 24.24, direction: 'N'});
